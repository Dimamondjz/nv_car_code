...
void pcd_grid_divider::setGridSize(double grid_size) {     //获得网格的大小
  if (grid_size < 1.0) {
...
std::vector<std::string> pcd_grid_divider::getAllFiles() {   //获取PCD文件
  // for single pcd file
  if (EndWith(_in_folder, ".pcd") || EndWith(_in_folder, ".PCD")) {
...
void pcd_grid_divider::check_point_type(const std::string &file) { //获取文件类型
...
bool pcd_grid_divider::run() {////地图分割主体
  ...
  // Search minimum and maximum points along x and y axis.                     查询地图的x,y轴最大最小长度
  ...
  // Find minimum and maximum boundary                                      查询到最小最大的边界
  ...
  // Number of grid along x and y axis                                      格子的大小
  ...
  std::vector<pcd_grid> grids(grid_num);                                   //一共有多少块
  ...
  // Assign all points to appropriate grid according to their x/y value   根据所有点的x,y数值进行分配适合的格子 要看
